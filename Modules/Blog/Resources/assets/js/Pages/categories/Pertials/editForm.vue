<script setup>
    ...
    const form = useForm({
        ...
        status: props.category.status ? true : false
    });

    const updateCategory = () => {
        form.put(route('blog-category.update', props.category.id), {
            errorBag: 'updateCategory',
            preserveScroll: true,
            onSuccess: () => {
                swal('Categoria actualizado con éxito.');
            },
        });
    };
        ...
        <template #actions>
            <Keypad>
                <template #botones>
                    ...
                    <Link :href="route('blog-category.index')"  class="inline-block px-6 py-2.5 bg-green-500 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-green-600 hover:shadow-lg focus:bg-green-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green-700 active:shadow-lg transition duration-150 ease-in-out ml-2">Ir al Listado</Link>
                </template>
            </Keypad>
        </template>
    </FormSection>
